...
import MessageContainer, {
  Props as MessageContainerProps,
} from '../message-container';
import ConversationContainer, {
  Props as ConversationContainerProps,
} from '../conversation-container';
import '../index.css';
...
const DesktopContent = styled.div`
  ...
  position: relative;
`;
const EmptyDesktopContent = styled.div`
  height: 100%;
  min-height: 100vh;
  width: 100%;
  position: relative;
...
export default function MainContainer({
  ...
  return (
    <Container>
      {useMobileView ? (
        selectedConversation ? (
          <MessageContainer {...selectedConversation} mobileView={mobileView} />
        ) : (
          <>
            <ConversationContainer {...inbox} />
          </>
        )
      ) : (
        <DesktopContainer>
          {inbox.loading ? (
            <Loading themeColor={inbox.themeColor} />
          ) : (
            <>
              <Sidebar>
                <ConversationContainer {...inbox} showHeader={false} />
              </Sidebar>
